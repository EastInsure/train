...
#bash -c '$LAUNCHER --node_rank ${NODE_RANK} $CMD' 2>&1 | tee /mnt/wubaodong/zhanmingjie/research/SenseLM-GPT_2023022403/train/logs/7B1_test_$(date +%Y%m%d%H%M%S).log
echo "logfile:$(dirname $0)/logs/${HOSTNAME}.log"

bash -c '$LAUNCHER --node_rank ${NODE_RANK} $CMD' 2>&1 | tee $(dirname $0)/logs/${HOSTNAME}.log
